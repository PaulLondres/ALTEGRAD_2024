...
def load_dataset():
    ...
    dataset = TUDataset(root='/tmp/MUTAG', name='MUTAG')  # '/tmp/MUTAG' est le chemin de téléchargement, modifiez-le si besoin
...
def graphlet_kernel(Gs_train, Gs_test, n_samples=200):
    ...
    for i, G in enumerate(Gs_train):
        for _ in range(n_samples):
            nodes = np.random.choice(list(G.nodes), 3, replace=False)
            subgraph = G.subgraph(nodes).to_undirected()
            for j, graphlet in enumerate(graphlets):
                #print(type(subgraph), type(graphlet))
...
print(f"Accuracy with graphlet kernel : {accuracy_gl}")